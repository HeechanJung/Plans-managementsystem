...
public class Planmanager implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 1009942656286042017L;
	...
	public Planmanager(Scanner input) {
		this.input = input;
	}
	public void addPlans(Plannerinput plannerinput) {
		...
		while (kind < 1 || kind > 3) {
			try {
				...
				if (kind == 1) {
					...
					Plannerinput = (manage.Plannerinput) new promission(PlanKind.Promission);
					Plannerinput.getUserInput(input);
					list.add(Plannerinput);
					break;
				} else {
...
//	System.out.println(list.size());
	...
	public Plannerinput get(int index) {
		return (Plannerinput) list.get(index);
	}
